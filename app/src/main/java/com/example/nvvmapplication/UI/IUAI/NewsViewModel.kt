...
//live data objects for search news response
    ...
    private suspend fun safeSearchNewsCall(searchQuery: String) {
        ...
        try {
            if(hasInternetConnection()){
                val response = newsRepository.searchNews(searchQuery,searchNewsPage)//make the network response and when this finish the coroutine will continue with the next line
                searchNews.postValue(handleSearchNewsResponse(response))
            }else{
